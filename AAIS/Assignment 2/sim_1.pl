...
initial('C:/Users/adamze/HUD MSc Repo/HUD MSc in AI/AAIS/Assignment 2/scenario1.pl') :- computer(windows).
...
% Coursework Question 1c code here:

change_plan(L, PLAN2) :-
        init(_, _, _, _, STATICS, PLAN, _, _),
        junction_name_end(L, J),
        cyc_length(J, CL),
        %calculate total stage time for the junction, this is needed for adjustment of stage 0
        total_stage_time(L, TST),
        %calculate total inter time as TIT = total cycle length - total stage lengths
        TIT is CL - TST,
        %The below assumes that there are only 2 stages in the junction, as per case study
        %to be fully generic, loop would have to be developed
        list_of_stages(J, [S0|[S1|_]]),
        total_flow_rate(L, S1, FL),
        member( equals(turnrate(fake, outside, L),Y),STATICS),
        %Calculate stage lenghths
        % SL1 is the calculation for stage 1 based on the flow
        SL1 is Y * CL / FL,
        %SL0 is the calculation for adjustment of stage 0 based on change in stage 1
        % as total cycle time - total inter - stage 1
        %This as well can be achieved by getting the current stage values from S0 and S1 and 
        %decrementing S0 by the value of increment of S1
        SL0 is CL - TIT - SL1,
        %this is ugly, but works
        %it should be extended to avoid repetition
        delete(PLAN,    equals(defaultgreentime(S1),_),  REDUCED_PLAN),
        PLAN1 = [equals(defaultgreentime(S1),SL1) | REDUCED_PLAN ],
        delete(PLAN1,    equals(defaultgreentime(S0),_),  REDUCED_PLAN2),
        PLAN2 = [equals(defaultgreentime(S0),SL0) | REDUCED_PLAN2 ].
        %write(PLAN1).




total_stage_time(L, ST) :-
        junction_name_end(L, J),
        list_of_stages(J, SList),
        acculmulate_stage(L,SList, ST).

        
acculmulate_stage(L, [S | SR], ST) :-
        %total_flow_rate(L,S, TFR),
        stage_len(S, STL),
        Stage_Flow is STL,
        acculmulate_stage(L,SR, ST1),
        ST is ST1 + Stage_Flow,
        !.
acculmulate_stage(_,[], 0). 

% End of coursework 1c
...
% Helper functions from week11_code.pl
%
read_in_state :- 
        initial(I), compile(I).
...
% End helper function from week11_code.pl
