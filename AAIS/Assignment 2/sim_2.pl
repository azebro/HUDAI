...
initial('C:/Users/adamze/HUD MSc Repo/HUD MSc in AI/AAIS/Assignment 2/scenario2.pl') :- computer(windows).
...
init_change_markers(I) :-
        I1 is 0 - I,
        assert(change_interval(I)),
        assert( remember_change(I1, albert_south) ),
        assert( remember_change(I1, callans_south) ),
        assert( remember_change(I1, hepton_south) ),
        assert( remember_change(I1, trades_north) ).
%remember_change(T, S).
occ_link(L, DYN, V) :- 
...
adjust_plan(BL, PLAN, STATICS, DYN, T, PLAN2) :-
        ...
        %Get link occupancy
        occ_link(BL, DYN, OV),
        % If queue is not more that 10, do nothing
        OV > 10,
        format("Change needed for ~w. Queue length: ~2f.~n", [BL, OV]),
        %Get junction that this link belongs to
        junction_name_end(BL, STATICS, J),
        %writeln(J),
        %Get stages defined for the junction
        %Given the HB model for coursework, I assume all junctions have only 2 satges
        % hence can directly unpack
        get_stages(J, STATICS, [S0|[S1|_]]),
        %Get max green time for stage 1, this is the upper boundary
        %I plan only to increase green time for S1, so no need to pull minimum green time
        member(equals(maxgreentime(S1),MV1),  STATICS),
        %Opposite is true for stage 0, where only minimum time is needed
        member(equals(mingreentime(S0),MV0),  STATICS),
        %Get current green times for stage 1 and 0 from the current plan
        member(equals(defaultgreentime(S1), DV1), PLAN),
        member(equals(defaultgreentime(S0), DV0), PLAN),
        %writeln(MV1),
        %writeln(MV0),
        ...
        DV1_2 is DV1 + 1,
        %decrease green time for stage 0 for the same value as increase of stage 1
        %I assume that we should keep the cycle length same
        DV0_2 is DV0 - 1,
        %writeln(DV1_2),
        %writeln(DV0_2),
        %Check if the new values do not violate the max (S1) and min (S0) green times
        ...
        delete(PLAN,    equals(defaultgreentime(S0),_),  REDUCED_PLAN),
        PLAN1 = [equals(defaultgreentime(S0),DV0_2) | REDUCED_PLAN ],
        delete(PLAN1,    equals(defaultgreentime(S1),_),  REDUCED_PLAN2),
        PLAN2 = [equals(defaultgreentime(S1),DV1_2) | REDUCED_PLAN2 ],
        retract(remember_change(RC, BL)),
        assert(remember_change(T, BL)),
        format("Change executed for ~w, new values: S0: ~d, S1: ~d.~n", [BL, DV0_2, DV1_2]).
...
%       waiting to turn on
        ...
        %This ideally should be implemented using list to avoid duplication
        adjust_plan(hepton_south, PLAN2, STATICS, DYN2, T, PLAN3),
        adjust_plan(albert_south, PLAN3, STATICS, DYN2, T, PLAN4),
        adjust_plan(callans_south, PLAN4, STATICS, DYN2, T,  PLAN5),
        adjust_plan(trades_north, PLAN5, STATICS, DYN2, T, PLAN6),
